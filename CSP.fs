...
module CSP =
    ...
    type estado<'variable,'valor when 'variable: comparison and 'valor   : comparison>
        = Map<'variable, 'valor list>

    type restriccion<'variable,'valor when 'variable: comparison and 'valor   : comparison>
        =  Binaria of ('variable * 'variable) * (estado<'variable,'valor> -> bool)
         | NAria of ('variable list) * (estado<'variable,'valor> -> bool)

    type csp<'variable,'valor when 'variable: comparison and 'valor   : comparison> =
        ...
        let remover estado (restriccion, (v,v')) =
            estado |> Map.find v
                    |> List.fold (fun (modificado, dom) x ->
                            ...
                            if List.exists (fun y -> let estado'' = Map.add v' [y] estado' eval restriccion estado'') (Map.find v' estado)
